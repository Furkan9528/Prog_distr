...
export const InvoiceUpdate = () => {
  ...
  const loading = useAppSelector(state => state.gateway.invoice.loading);
  const updating = useAppSelector(state => state.gateway.invoice.updating);
  const updateSuccess = useAppSelector(state => state.gateway.invoice.updateSuccess);
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="gatewayApp.invoiceInvoice.home.createOrEditLabel" data-cy="InvoiceCreateUpdateHeading">
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.code')}
                id="invoice-code"
                name="code"
                data-cy="code"
                type="text"
              ...
              />
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.date')}
                id="invoice-date"
                name="date"
                data-cy="date"
                type="datetime-local"
              ...
              />
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.details')}
                id="invoice-details"
                name="details"
                data-cy="details"
                type="text"
              />
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.status')}
                id="invoice-status"
                name="status"
                data-cy="status"
                type="select"
              >
                {invoiceStatusValues.map(invoiceStatus => (
                  <option value={invoiceStatus} key={invoiceStatus}>
                    {translate('gatewayApp.InvoiceStatus.' + invoiceStatus)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.paymentMethod')}
              ...
              >
                {paymentMethodValues.map(paymentMethod => (
                  <option value={paymentMethod} key={paymentMethod}>
                    {translate('gatewayApp.PaymentMethod.' + paymentMethod)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.paymentDate')}
              ...
              />
              <ValidatedField
                label={translate('gatewayApp.invoiceInvoice.paymentAmount')}
